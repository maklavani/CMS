...
class Widgets {
		...
		if(is_array($widgets))
			foreach ($widgets as $key => $widget_item)
				if(User::has_permission($widget_item->permission) && self::in_page($widget_item->menu_type , $widget_item->menus)){
					ob_start();
					$setting = json_decode($widget_item->setting);
					$widget = $widget_item;

					if(System::has_file(_ADM . 'widgets/' . $widget_item->type . '/details.json')){
	...
	private static function set_active_menu()
	{
		...
		else
		{
			...
			$db->table('menu')->where('`link` = "' . htmlspecialchars_decode(str_replace(Site::$base . _ADM , "" , Site::$full_link)) . '"')->select()->process();
			$menu = $db->output();
