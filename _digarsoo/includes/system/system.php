...
class System {
	...
	{
		if(file_exists($file = _SRC_SITE . $check_source))
		{
			if($read)
				require_once $file;
			return true;
	...
	{
		$src = array();
		$inners = scandir(_SRC_SITE . $source);
	...
	public static function print_path($source)
	{
		...
		$directory = glob(_SRC_SITE . $source . '*' , GLOB_ONLYDIR);
		$file_paths = glob(_SRC_SITE . $source . '*.*' , GLOB_BRACE | GLOB_NOSORT);

		if(User::$group == 2)
		{
			$htaccess = glob(_SRC_SITE . $source . '.htaccess' , GLOB_BRACE | GLOB_NOSORT);
	...
	public static function is_deleted($srsc)
	{
		...
		foreach ($srsc as $value){
			...
			{
				$setting = json_decode(file_get_contents(_SRC_SITE . $value . '.' . _COPR));
	...
	public static function delete_files($srsc)
	{
		...
		if($srsc != "" && $srsc != "/")
			foreach ($srsc as  $value) {
				if (is_dir(_SRC_SITE . $value) === true){
					$inners = array_diff(scandir(_SRC_SITE . $value) , array('.' , '..'));
					...
					if(System::has_file($value . '.' . _COPR))
						$result += !unlink(_SRC_SITE . $value . '.' . _COPR);

					$result += !rmdir(_SRC_SITE . $value);
				}

				else if (is_file(_SRC_SITE . $value) === true)
				{
					if(System::has_file($value . '.' . _COPR))
						$result += !unlink(_SRC_SITE . $value . '.' . _COPR);

					$result += !unlink(_SRC_SITE . $value);
				}
			}
	...
	public static function rename($new_name , $src)
	{
		...
		if(is_dir(_SRC_SITE . $src) === true)
		{
			if(System::has_file($dir . '/' . $new_name))
				return false;
			else
				return rename(_SRC_SITE . $src , _SRC_SITE . $dir . '/' . $new_name);
		}
		else
		{
			$dot_pos = strrpos($base , '.' , -1);
			...
			else
			{
				if(System::has_file($src . '.' . _COPR))
					rename(_SRC_SITE . $src . '.' . _COPR , _SRC_SITE . $dir . '/' . $new_base . '.' . _COPR);

				return rename(_SRC_SITE . $src , _SRC_SITE . $dir . '/' . $new_base);
			}
		}
	}
	...
	public static function move($new_name , $src)
	{
		if(System::has_file($new_name))
			...
			return rename(_SRC_SITE . $src , _SRC_SITE . $new_name);
	}
	...
	public static function copies($address , $source)
	{
		...
		foreach ($address as $path)
			...
			{
				$base_first = $base = basename(_SRC_SITE . $path);
				...
				if($path != $source . $base && $path != $source . $base . '/')
				{
					...
					while(true){
						...
						{
							if(is_dir(_SRC_SITE . $source . $base) === true){
								$number++;
					...
					if(is_dir(_SRC_SITE . $path) === true)
					{
						$inners = scandir(_SRC_SITE . $path);
						$result += !mkdir(_SRC_SITE . $source . $base , 0755 , true);
					...
					else if(is_file(_SRC_SITE . $path) === true)
					{
						if(System::has_file($path . '.' . _COPR) && basename($path) != '.' . _COPR && !in_array($path . '.' . _COPR , $address))
							$result += !copy(_SRC_SITE . $path . '.' . _COPR , _SRC_SITE . $source . $base . '.' . _COPR);
							
						$result += !copy(_SRC_SITE . $path , _SRC_SITE . $source . $base);
					}
				}
			}
		return $result;
	...
	public static function archive($address , $source , $name)
	{
		...
		if($zip->open(_SRC_SITE . $source . $filename , ZipArchive::CREATE) !== false){
			foreach ($address as $path)
				if(System::has_file($path))
				{
					if(is_dir(_SRC_SITE . $path) === true){
						...
						if(!empty($inners))
							foreach ($inners as $valueb)
								$result += !$zip->addFile(_SRC_SITE . $valueb , str_replace(dirname(_SRC_SITE . $path) . '/' , "" , _SRC_SITE . $valueb));
					}
					else
						$result += !$zip->addFile(_SRC_SITE . $path , basename($path));
				}
	...
	public static function extract($file , $source , $download_file = true)
	{
		...
		if($zip->open(_SRC_SITE . $file) === TRUE)
		{
			$result = $zip->extractTo(_SRC_SITE . $source . $filename . '/');
			$zip->close();
	...
	public static function new_folder($folder , $source , $download_file = true)
	{
		...
		foreach ($folders as $value)
			if(!System::has_file($source . $value)){
				if(mkdir(_SRC_SITE . $source . $value , 0755 , true)){
					$html_file = fopen(_SRC_SITE . $source . $value . "/index.html" , "w");
					fwrite($html_file , "<!DOCTYPE html><title></title>");
					fclose($html_file);

					if($download_file)
					{
						$file = fopen(_SRC_SITE . $source . $value . '/.' . _COPR , "w");
	...
	public static function new_file($file , $source)
	{
		...
		{
			$new_file = fopen(_SRC_SITE . $source . $file , "w");
			fclose($new_file);

			$file = fopen(_SRC_SITE . $source . $file . '.' . _COPR , "w");
			fwrite($file , json_encode(array('permission' => 5 , 'downloads' => 0 , 'deleted' => 1 , 'users' => array('-1')) , JSON_PRETTY_PRINT));
			fclose($file);
		}
		else
	...
	{
		if(is_dir(_SRC_SITE . $source) === true)
		{
			$inners = scandir(_SRC_SITE . $source);
			...
			{
				$html_file = fopen(_SRC_SITE . $source . "/index.html" , "w");
				fwrite($html_file , "<!DOCTYPE html><title></title>");
				fclose($html_file);
