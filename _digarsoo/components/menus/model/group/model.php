...
class GroupModel extends Model {
	...
	{
		$this->table('menu')->where('`homepage` = 1 AND `group` = ' . $group)->select()->process();
		return $this->output();
	}
	...
	public function set_index($group)
	{
		$parent = 0;
		$index = 1;
		$this->table('menu')->where('`group` = ' . $group)->order('`parent` ASC , `index` ASC')->select()->process();
		...
		if($menus = $this->output())
			foreach($menus as $key => $value){
				...
				if($value->index != $index)
					$this->table('menu')->update(array(array('index' , $index)))->where('`id` = ' . $value->id)->process();
	...
	public function save()
	{
		...
		$this->table('menu')->where('`group` = ' . $group[0]->id . ' AND `parent` = ' . $_POST['field_input_parent'])->select()->process();

		if($menus = $this->output())
			foreach ($menus as $value)
				if($value->index >= $_POST['field_input_index'])
					$this->table('menu')->update(array(array('index' , $value->index + 1)))->where('`id` = ' . $value->id)->process();

		$setting = array('title' => $_POST['field_input_title'] , 'show_status' => $_POST['field_input_show_status'] , 'class' => $_POST['field_input_class']);

		$this->table('menu');
		$this->insert(	array(	'name' , 'status' , 'type' , 'link' , 'languages' , 'parent' , 'group' , 
								'permission' , 'index' , 'icon' , 'homepage' , 'meta_tag' , 'meta_description' , 'setting' , 
								'location') , 
	...
	public function update_menu()
	{
		...
		$this->update(	array(
							...
							array('index' , $_POST['field_input_index']) , 
