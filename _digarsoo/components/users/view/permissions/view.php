...
class PermissionsView extends View {
	public function base($params)
	{
		...
		{
			Templates::$title = Language::_('COM_USERS_PERMISSIONS');
			self::set_toolbar(array('title' => Templates::$title , 'buttons' => array('delete' , 'edit' , 'new')));
			self::set_search(array('sort' => array('name' , 'groups' , 'id')));
		}
		else if(Controller::$action == 'new')
		{
			Templates::$title = Language::_('COM_USERS_PERMISSIONS') . ' - ' . Language::_('NEW');
			self::set_toolbar(array('title' => Templates::$title , 'buttons' => array('close' , 'saveclose' , 'savenew' , 'save')));
		}
		else if(Controller::$action == 'edit')
		{
			Templates::$title = Language::_('COM_USERS_PERMISSIONS') . ' - ' . Language::_('EDIT');
