...
class ComponentsView extends View {
	public function base($params)
	{
		...
		{
			Templates::$title = Language::_('COM_EXTENSION_COMPONENTS');
			self::set_toolbar(array('title' => Templates::$title , 'buttons' => array('unblock' , 'block' , 'edit')));
			self::set_search(array('item' => array('name') , 'sort' => array('name' , 'id')));
		}
		else if(Controller::$action == 'edit')
		{
			Templates::$title = Language::_('COM_EXTENSION_COMPONENTS') . ' - ' . Language::_('EDIT');
