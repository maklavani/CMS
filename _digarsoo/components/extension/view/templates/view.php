...
class TemplatesView extends View {
	public function base($params)
	{
		...
		{
			Templates::$title = Language::_('COM_EXTENSION_TEMPLATES');
			self::set_toolbar(array('title' => Templates::$title , 'buttons' => array('edit')));
			self::set_search(array('item' => array('name') , 'sort' => array('name' , 'location' , 'id')));
		}
		else if(Controller::$action == 'edit')
		{
			Templates::$title = Language::_('COM_EXTENSION_TEMPLATES') . ' - ' . Language::_('EDIT');
