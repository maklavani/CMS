...
class Menus {
		...
		foreach ($menus as $key => $value)
			if($value->parent == $parent && User::has_permission($value->permission))
			{
				...
				if($value->id == Widgets::$active_menu)
				{
					...
					$setting = json_decode(htmlspecialchars_decode($value->setting));

					if($value->meta_tag != "")
						Templates::add_meta('name' , 'keywords' , 'content' , $value->meta_tag);

					if($value->meta_description != "")
						Templates::add_meta('name' , 'description' , 'content' , $value->meta_description);

					if($value->robots_index != "off" || $value->robots_follow != "off")
					{
						$robots = "";
						if($value->robots_index != "off")
							$robots .= $value->robots_index;

						if($value->robots_follow != "off")
							$robots .= ($robots != "" ? ", " : "") . $value->robots_follow;

						Templates::add_meta('name' , 'robots' , 'content' , $robots);
					}
				}
	...
	// gereftane name menue active
	public static function get_active_menu_name()
	{
		$db = new Database;
		$db->table('menu')->where('`id` = ' . Widgets::$active_menu)->select()->process();
	...
	private function output_child($menus , $space)
	{
		...
		if(!empty($menus))
			foreach ($menus as $key => $value){
				$setting = isset($value['node']->setting) ? json_decode(htmlspecialchars_decode($value['node']->setting)) : array('title' => '' , 'show_status' => '1' , 'class' => '');
				...
				if(isset($setting->attributes))
					$attributes .= " attributes=\"" . $setting->attributes . "\"";

				if($value['node']->link == "#")
					$output .= $space . "\t<a href=\"#\" " . $attributes . ">";
				else if($value['node']->homepage)
					$output .= $space . "\t<a href=\"" . Site::$base . (Language::$abbreviation ? Language::$abbreviation . '/' : "") . "\" " . $attributes . ">";
				else if(strpos($value['node']->link , "/") !== false)
					$output .= $space . "\t<a href=\"" . $value['node']->link . "\" " . $attributes . ">";
				else if(isset($value['node']->alias) && $value['node']->alias != "")
					$output .= $space . "\t<a href=\"" . Site::$base . (Language::$abbreviation ? Language::$abbreviation . '/' : "") . $value['node']->alias . "\" " . $attributes . ">";
				else
					$output .= $space . "\t<a href=\"" . (Language::$abbreviation ? Language::$abbreviation . '/' : "") . $value['node']->link . "\" " . $attributes . ">";
